...
proc eventLoop(
  ...
  let server = if settings.domain == Domain.AF_UNIX:
    newSocket(settings.domain, SOCK_STREAM, Protocol.IPPROTO_IP)
  else:
    newSocket(settings.domain)
  if settings.domain != Domain.AF_UNIX:
    server.setSockOpt(OptReuseAddr, true)
  if compileOption("threads") and not settings.reusePort and settings.domain != Domain.AF_UNIX:
  ...
  if settings.domain in {Domain.AF_INET, Domain.AF_INET6}:
    ...
    try:
      ...
      if e.errorCode == ENOPROTOOPT:
        echo("SO_REUSEPORT not supported on this platform.")
      else: raise

  if settings.domain == Domain.AF_UNIX:
    if settings.bindAddr.len == 0:
      raise newException(ValueError, "UDS requires bindAddr (socket path)")
    discard tryRemoveFile(settings.bindAddr)
    var sun: Sockaddr_un
    sun.sun_family = cast[TSa_Family](posix.AF_UNIX)
    if settings.bindAddr.len >= sun.sun_path.len:
      raise newException(ValueError, "UDS path too long: " & settings.bindAddr)
    for i, c in settings.bindAddr:
      sun.sun_path[i] = c
    sun.sun_path[settings.bindAddr.len] = '\0'
    let sunSize = SockLen(sizeof(sun))
    if posix.bindSocket(server.getFd(), cast[ptr SockAddr](addr sun), sunSize) < 0:
      raiseOSError(osLastError(), "bind UDS " & settings.bindAddr)
    discard posix.chmod(settings.bindAddr.cstring, 0o666.Mode)
  else:
